...
{
    if (sectionCaches.contains(swc))
    {
        return sectionCaches[swc];
    }
    return nullptr;
}

void BlockSystem::RemoveSectionCache(i32vec2 swc) { sectionCaches.erase(swc); }
...
Opt<Block> BlockSystem::GetBlock(glm::i32vec3 bwc)
{
    ...
    // 边界检查
    if (bwc.x < lbwc.x ||
        bwc.x >= lbwc.x + lattice.GetFullSize() * Section::Size ||
        bwc.z < lbwc.y ||
        bwc.z >= lbwc.y + lattice.GetFullSize() * Section::Size || bwc.y < 0 ||
        bwc.y >= Section::Height)
    {
        return nullopt;
    }

    glm::i32vec3 slc = {(bwc.x - lbwc.x) / Section::Size, 0,
                        (bwc.z - lbwc.y) / Section::Size};
    glm::i32vec3 bsc = {(bwc.x - lbwc.x) % Section::Size, bwc.y,
                        (bwc.z - lbwc.y) % Section::Size};
...
void BlockSystem::SetBlock(Block block, i32vec3 bwc)
{
    ...
    swc.x = bwc.x >= 0 ? bwc.x / Section::Size
                       : (bwc.x - Section::Size + 1) / Section::Size;

    swc.y = bwc.z >= 0 ? bwc.z / Section::Size
...
void BlockSystem::PrintAllSwc()
{
    for (auto each : sectionCaches)
    {
        Debug::Log(each.first);
    }
}